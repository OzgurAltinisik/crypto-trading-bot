...
# --- Signals (matches live bot logic, with EMA200 slope filter) ---
df["ema200_slope"] = df["ema200"] > df["ema200"].shift(10)
df["buy"]  = (df["ema9"] > df["ema21"]) & (df["ema9"].shift(1) <= df["ema21"].shift(1)) & (df["rsi"] < 60) & (df["close"] > df["ema200"]) & (df["ema200_slope"])
...
# --- Results ---
print("\n===== BTC/USDT BACKTEST (EMA 9/21 + ATR + EMA200 slope filter) =====")
if trades:
...
ax1.set_title(f"BTC/USDT 1H | EMA 9/21 + ATR (SL×{ATR_SL} / TP×{ATR_TP})", color="white")
